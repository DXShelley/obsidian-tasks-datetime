...
1. `manifest.json` uses `tasks-datetime`, which does not contain the reserved term `obsidian`. Its uniqueness must still be verified immediately before submission.
...
Thus, GitHub publication can continue after the actions below; Community Plugins submission must still wait for the required upstream authorization (or the documented alternative condition).
...
| --- | --- | --- |
| `manifest.json` | `id`: `tasks-datetime`; `name`: `Tasks Datetime`; `authorUrl`: `https://github.com/DXShelley` | ID does not contain the reserved term `obsidian`; uniqueness remains a point-in-time check. |
...
| Official directory index | No entry with ID `tasks-datetime` and no entry named `Tasks Datetime` at the audit snapshot | This is only a point-in-time uniqueness check, not a reservation. Re-check immediately before submission. |
...
| Community Plugins ID | The [submission guide](https://docs.obsidian.md/Plugins/Releasing/Submit+your+plugin) says the ID must be unique across published plugins **and cannot contain `obsidian`**. | Medium until a fresh uniqueness check is made immediately before submission. | `tasks-datetime` removes the reserved term. Migrate local installations deliberately and verify uniqueness before submission. |
